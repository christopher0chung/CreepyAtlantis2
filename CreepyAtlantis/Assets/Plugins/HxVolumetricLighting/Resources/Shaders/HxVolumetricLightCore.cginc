...
float Noise(float3 x)
{
	...
	return ((tex3Dlod(NoiseTexture3D, float4(x.x * NoiseScale.x, x.y * NoiseScale.y, x.z * NoiseScale.z, 1)).a - 0.5f) * max(hxNoiseContrast,0)) + 0.5f;
#else
	return 1;
#endif
}
...
float GetFogDensity(float3 worldPosition)
{
	...
	fogDensity = 1.75*Noise(worldPosition) * fogDensity;

	return fogDensity * Density.x;
#else
	return Noise(worldPosition) * Density.x;
#endif
}
