...
public class PlayerAir : MonoBehaviour {
    ...
    [SerializeField] private float _airTankPercent;
    public float airTankPercent
    {
        ...
        set
        {
            if (_airTankPercent != value)
            {
                _airTankPercent = value;
                Speaker who;
                if (transform.root.name == "Character0")
                    who = Speaker.Ops;
                else
                    who = Speaker.Doc;
                EventManager.instance.Fire(new GE_Air(who, _airTankPercent / 100));
            }
            //_airTankPercent = value;
