...
#if UNITY_5_4_OR_NEWER && !UNITY_5_4_0
...
public class HxVolumetricCamera : MonoBehaviour
{
    ...
    [Range(0, 1)]
    public float FeedbackMin = 0.1f;
    [Range(0, 1)]
    public float FeedbackMax = 0.7f;
    [Range(0, 4)]
    ...
    {
        if (cam != ActiveCamera) { ReleaseLightBuffers(); }
    }
   ...
   // bool warned = false;
    ...
    void CreateTempTextures()
    {
        ...
        if (downScaledBlurRT == null && ((blurCount > 0 || ((BlurTransparency > 0 || (MapToLDR == true || Mycamera.hdr == false)) && TransparencySupport)) && resolution != Resolution.full))
        {
        ...
        if (FullBlurRT2 == null && ((resolution != Resolution.full && UpSampledblurCount > 0) || (resolution == Resolution.full && (blurCount > 0 || ((BlurTransparency > 0 || (MapToLDR == true || Mycamera.hdr == false)) && TransparencySupport) || TemporalSampling)) || (MapToLDR || Mycamera.hdr == false)))
        {
           ...
           // { BufferFinalize.Clear(); }
            ...
            if ((BlurTransparency > 0 || (MapToLDR == true || Mycamera.hdr == false)) && TransparencySupport)
            {
                ...
                for (int i = 0; i < ctb + 1; i++)
                {
                    ...
                    for (int p = 0; p < tbc; p++)
                    {
                        ...
                        BufferFinalize.DrawMesh(HxVolumetricCamera.QuadMesh, Matrix4x4.identity, TransparencyBlurMaterial, 0, (((Mycamera.hdr == false || MapToLDR) && p == tbc - 1) ? 2 : 1));
                    }
            ...
            if (resolution != Resolution.full)
            {
                if (TemporalSampling)
                {
                    ...
                    BufferFinalize.SetGlobalVector("hxTemporalSettings", new Vector4(FeedbackMin, FeedbackMax, 0, 0));
                }
    ...
    void SaveUsedShaderVarience()
    {
        ...
        if (Application.isPlaying)
        {
            HxVolumetricShadersUsed.SetVolumetricValues(FullUsed, LowResUsed, HeightFogUsed, HeightFogOffUsed, NoiseUsed, NoiseOffUsed, TransparencyUsed, TransparencyOffUsed, DensityParticlesUsed, PointUsed, SpotUsed, DirectionalUsed, SinglePassStereoUsed);
        }
#endif
    }
    ...
    void CalculateEvent()
    {
        switch (Mycamera.actualRenderingPath)
        {
            ...
            case RenderingPath.Forward:
                ...
                if (Mycamera.depthTextureMode == DepthTextureMode.Depth) { RenderEvent = CameraEvent.BeforeDepthTexture; SetupEvent = CameraEvent.AfterDepthTexture; }
                if (Mycamera.depthTextureMode == DepthTextureMode.DepthNormals) { RenderEvent = CameraEvent.BeforeDepthNormalsTexture; SetupEvent = CameraEvent.AfterDepthNormalsTexture; }
                FinalizeEvent = CameraEvent.AfterForwardOpaque;
                LightRenderEvent = CameraEvent.AfterForwardOpaque;
                break;
        }
    }

#if HxApplyImageEffect
    [ImageEffectOpaque]
    ...
    void CheckTemporalTextures()
    {
        if (TemporalSampling)
        {
            ...
            {
                TemporalTexture = RenderTexture.GetTemporary(GetCamPixelWidth(), GetCamPixelHeight(), 0, RenderTextureFormat.ARGBHalf, RenderTextureReadWrite.Linear);
    ...
    void CreateLightbuffers()
    {
        ...
        ActiveCamera.AddCommandBuffer(LightRenderEvent, BufferRenderLights);
